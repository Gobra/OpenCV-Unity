...
CVAPI(void) calib3d_solvePnP_vector(
	...
	int useExtrinsicGuess, int flags)
{
	cv::Mat objectPointsMat(objectPointsLength, 1, cv::DataType<cv::Point3f>::type, objectPoints);
	cv::Mat imagePointsMat(imagePointsLength, 1, cv::DataType<cv::Point2f>::type, imagePoints);
...
CVAPI(void) calib3d_solvePnPRansac_vector(
	cv::Point3f *objectPoints, int objectPointsLength,
	cv::Point2f *imagePoints, int imagePointsLength,
	cv::Mat *cameraMatrix,
	double *distCoeffs, int distCoeffsLength,
	...
	int iterationsCount, float reprojectionError, double confidence, std::vector<int> *inliers,
	int flags)
{
